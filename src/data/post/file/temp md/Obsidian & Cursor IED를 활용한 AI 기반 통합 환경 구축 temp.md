...
**카테고리**와 **태그** 체계도 사전에 정의했습니다. 카테고리는 글을 크게 분류하는 범주로, 한 글당 하나의 카테고리를 지정하고 있습니다. 태그는 글의 주요 키워드이나 세부 주제를 나타내며 여러 개를 지정할 수 있습니다.

예를 들어 위 YAML에서:

- `category: "DevOps"`는 해당 글이 DevOps 관련 주제임을 표시
- `tags: ["AI", "Obsidian"]`은 AI와 Obsidian에 관한 내용이 포함되었음을 보여줍니다

이러한 메타데이터를 활용하면 블로그 상에서 동일 카테고리나 태그를 가진 글들을 묶어서 보여줄 수 있어, 독자나 작성자 모두 관련 글을 쉽게 탐색할 수 있습니다.
